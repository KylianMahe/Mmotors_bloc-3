...
async function requestBlob(path) {
  ...
  if (!response.ok) {
    let message = "Impossible de récupérer le document";
...
export const api = {
  ...
  getApplicationDetail: (applicationId) => request(`/applications/${applicationId}`),

  downloadDocument: (documentId) => requestBlob(`/applications/documents/${documentId}/download`),
  ...
  createVehicle: (payload) =>
    request("/admin/vehicles", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  switchVehicle: (vehicleId) =>
    request(`/admin/vehicles/${vehicleId}/switch`, {
      method: "PATCH",
    }),
